 ...
 * - prompt: clean user prompt before prependContext injection
...
// Process-level singleton to avoid concurrent cleanup races caused by
// starting the cleaner multiple times in the same process
...
export default function register(api: OpenClawPluginApi) {
  ...
  // TODO: implement hard per-turn call limit via before_tool_call hook + execute early-return (Approach D)
  ...
  if (cfg.recall.enabled) {
    ...
    api.on("before_prompt_build", async (event, ctx) => {
      ...
      if (!userText) {
        api.logger.debug?.(`${TAG} [before_prompt_build] No user prompt found, skipping recall`);
        return;
      }
