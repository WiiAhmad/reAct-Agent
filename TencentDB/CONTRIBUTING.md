...
│   ├── adapters/            # Host framework adapters
│   ├── cli/                 # Command-line utilities
│   ├── core/                # Memory pipeline and domain logic
│   ├── gateway/             # Gateway server and runtime integration
│   ├── offload/             # Context offload and compression logic
│   └── utils/               # Shared utilities
├── scripts/                 # Helper scripts (Gateway control, Hermes setup, etc.)
...
| Branch | Purpose |
|--------|---------|
| `master` | Default branch, PR target |
...
Use one of the following common contribution types, aligned with the PR change type where applicable:
...
`adapters`, `cli`, `core`, `gateway`, `offload`, `utils`, `hermes`
...
```bash
git commit -s -m "feat(core): add batch insert support"
```
