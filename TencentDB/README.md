...
Choose the integration path that matches your agent runtime: install the OpenClaw plugin directly, or add TencentDB Agent Memory to an existing Hermes installation with the provided setup script.
...
#### 1.3 Enable short-term compression (optional, requires plugin package version ≥ 0.3.4)
...
### 2. Hermes (existing install, requires TencentDB Agent Memory package version ≥ 0.3.4)

In addition to OpenClaw, this plugin also supports [Hermes](https://github.com/NousResearch/hermes-agent). This repository does not include a Docker image or Dockerfile for Hermes; the supported flow is to install Hermes first, then run the setup script below.

```bash
# From the TencentDB Agent Memory repo root:
bash scripts/install_hermes_memory_tencentdb.sh
```

The script downloads the latest `@tencentdb-agent-memory/memory-tencentdb` npm package and extracts it to `$TDAI_INSTALL_DIR` (default: `~/.memory-tencentdb/tdai-memory-openclaw-plugin`). It then runs `npm install --omit=dev` in that downloaded package, links Hermes to the bundled adapter at `$TDAI_INSTALL_DIR/hermes-plugin/memory/memory_tencentdb`, and writes Gateway environment variables to `~/.hermes/.env` for systemd-managed Hermes and `/etc/profile.d/memory-tencentdb-env.sh` for interactive shells.

After the script completes, enable the provider in `~/.hermes/config.yaml` if it is not already set:
...
Then configure LLM credentials in `~/.hermes/.env`:

```bash
MEMORY_TENCENTDB_LLM_API_KEY=your-api-key
MEMORY_TENCENTDB_LLM_BASE_URL=https://api.openai.com/v1
MEMORY_TENCENTDB_LLM_MODEL=gpt-4o
```

Restart Hermes or start a new shell so it picks up the updated configuration. The Gateway defaults to `127.0.0.1:8420`.
...
| :--- | :--- | :--- |
| `storeBackend` | `"sqlite"` | Storage backend: `sqlite` (local SQLite + sqlite-vec) or `tcvdb` (Tencent Cloud Vector Database) |
...
- **Prefer WeChat?** Scan the QR code above to join the Agent Memory WeChat community and talk with early contributors.
