...
export class LocalMemoryCleaner {
  ...
  async runOnce(nowMs = Date.now()): Promise<void> {
    ...
    // For example, if retentionDays=2 and today is 03-15, keep 03-14 and 03-15,
    // and delete records older than 03-14 00:00:00.000 local time.
...
function computeCutoffMsByLocalDay(nowMs: number, retentionDays: number): number {
  ...
  // The deletion cutoff is keepStart at 00:00:00.000 in local time.
  const now = new Date(nowMs);
