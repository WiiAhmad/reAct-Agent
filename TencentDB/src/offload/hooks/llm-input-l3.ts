 ...
 *    message with tool_use blocks plus all corresponding toolResult messages.
 * 2. Score each group by total token count.
 * 3. Delete the largest group. Repeat until below target.
 *
 * Non-tool messages such as plain assistant replies are also candidates and are
 * treated as single-message groups.
 */
...
function buildHistoryMmdText(filename: string, mmdContent: string): string {
  ...
  return [
    ...
    `Historical task context. Below is the status diagram of a completed or paused historical task.`,
    taskGoal ? `**Task goal:** ${taskGoal}` : "",
    ``, "```mermaid", mmdContent, "```", `</history_task_context>`,
  ].filter((line) => line !== "").join("\n");
}

/** Compact metadata-only version when the full MMD exceeds the token budget */
function buildHistoryMmdMetaText(filename: string, mmdContent: string): string {
  ...
  return [
    ...
    `Historical task summary. Below is the metadata for a historical task. The original diagram has been omitted to save context.`,
    taskGoal ? `**Task goal:** ${taskGoal}` : "",
    `**Task file:** ${filename}`,
...
async function fastPathReApply(messages: any[], stateManager: OffloadStateManager, logger: PluginLogger): Promise<{ applied: number; deleted: number }> {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    // FIX: For mixed assistant messages (content blocks + tool_use), strip deleted tool_use
