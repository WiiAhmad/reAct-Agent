...
export function createAfterToolCallHandler(
  ...
  return async (event: any, ctx: any) => {
    ...
    // tool results that happen to contain "Approval required" in their output.
...
async function checkAndCompressAfterToolCall(
  ...
  try {
    ...
    if (workingTokens >= mildThreshold) {
      ...
      logger.info(`[context-offload] L3(after_tool_call) MILD done: replaced=${cascadeResult.replacedCount}, threshold=${cascadeResult.finalThreshold}${detailStr ? `, items=[${detailStr}]` : ""}`);
