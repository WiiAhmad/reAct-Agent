...
async function buildActiveMmdBlock(
  ...
  try {
    ...
    return [
      ...
      `Mermaid flowchart for the current active task. This is a staged record of the task you have been working on most recently. Tool use appearing below this message has not yet been summarized, so the view may lag and is for reference only.`,
      taskGoal ? `**Task goal:** ${taskGoal}` : "",
      `**Task file:** ${activeMmdFile}`,
      nodeIds.length > 0
        ? `**Node index:** You can look up the corresponding tool call record in offload.{sessionid}.jsonl by node_id. If you need the original tool call and full result for a specific node, find that entry's result_ref in offload.{sessionid}.jsonl and read the referenced file.`
        : "",
      "```mermaid",
      mmdContent,
      "```",
      `Nodes marked "doing" are the current focus. Note that the view may lag because tool use below this message has not yet been counted, so use it only as a reference. Nodes marked "done" are already completed. Use this to stay oriented and avoid repeating finished work.`,
      `</current_task_context>`,
    ]
      .filter((line) => line !== "")
      .join("\n");
