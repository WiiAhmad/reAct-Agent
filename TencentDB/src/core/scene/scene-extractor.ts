 ...
 *   5. Parse LLM output for out-of-band persona update signals
 */
...
/**
 * Parse LLM output for a persona update request signal.
 *
...
export function parsePersonaUpdateSignal(text: string): { reason: string } | null {
  ...
  // Inline format: PERSONA_UPDATE_REQUEST: reason message
...
export class SceneExtractor {
  ...
  async extract(memories: Array<{ content: string; created_at: string; id?: string }>): Promise<ExtractionResult> {
    ...
    if (sceneCount >= this.maxScenes) {
      ...
      sceneCountWarning = `Current scene count is **${sceneCount}**. Prefer UPDATE operations or proactively MERGE similar scenes.`;
   ...
   *
   * Includes a capacity counter at the top (for example, "Current scene count: 5 / 15")
  ...
  private buildSceneSummaries(
    ...
    // Inject the capacity counter at the top — the LLM sees this first
