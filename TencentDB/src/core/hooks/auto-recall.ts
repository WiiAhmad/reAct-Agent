...
- **tdai_memory_search**: Search structured memories (L1). Use it to recall key information such as user preferences, historical event milestones, and rules.
- **tdai_conversation_search**: Search raw conversations (L0). Use it to find exact message text, timelines, and contextual details. It can also supplement or verify the results from tdai_memory_search.
- **read_file** (paths from Scene Navigation): Use it after locating the relevant scene when you need the full scene profile, event progression, or stage conclusions.

### ⚠️ Call limits
Across one conversation turn, \`tdai_memory_search\` and \`tdai_conversation_search\` may be called at most 3 times in total.
- If the first search returns no results, you may retry with different keywords or switch tools, but do not exceed 3 total calls.
- If there are still no results after 3 searches, the information is not in memory. Reply directly based on the information already available and do not keep searching.
</memory-tools-guide>`
 ...
 *
 * Time semantics:
 *   - timestamp (point-in-time): when the activity or event happened, e.g. "2025-03-01 mentioned something"
 *   - activity_start_time / activity_end_time (time span): activity time range, e.g. "trip from 2025-05-01 to 2025-05-10"
 *   - All three time fields may be empty/undefined — handled gracefully.
 *
 * Output examples:
 *   - [persona] The user is named Wang Xiaoming, is 30 years old, and works as a software engineer.
 *   - [episodic|travel plan] The user plans to travel to Japan in May. (Activity time: 2025-05-01 ~ 2025-05-10)
 *   - [episodic] The user worked late today. (Activity time: 2025-03-01)
 *   - [instruction] The user wants replies in Chinese and prefers concise answers.
 */
interface FormatableMemory {
  type: string;
  content: string;
  scene_name?: string;
  /** Activity time range start (time span start), may be empty */
  activity_start_time?: string;
  /** Activity time range end (time span end), may be empty */
  activity_end_time?: string;
  /** Activity point-in-time (when it happened), may be empty */
  timestamp?: string;
}
...
function formatMemoryLine(m: FormatableMemory): string {
  ...
  if (start && end) {
    // Time span: both start and end
    line += ` (Activity time: ${start} ~ ${end})`;
  } else if (start) {
    // Time span: only start
