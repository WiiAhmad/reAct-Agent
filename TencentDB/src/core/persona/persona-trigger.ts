  ...
  async shouldGenerate(): Promise<TriggerResult> {
    ...
    if (cp.request_persona_update) {
      const result: TriggerResult = {
        should: true,
        reason: `Manual request: ${cp.persona_update_reason || "Agent requested an update"}`,
      };
    ...
    if (
      ...
      const result: TriggerResult = { should: true, reason: "First cold start: initial extraction completed and scene files exist" };
