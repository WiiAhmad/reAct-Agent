  ...
  async generateLocalPersona(triggerReason?: string): Promise<boolean> {
    ...
    if (changedSceneContents.length > 0) {
      changedScenesContent =
        ...
        `*Since the last Persona update, the following ${changedSceneContents.length} scenes have changed. The system has preloaded their full content for you:*\n\n` +
        changedSceneContents.join("\n\n") +
        `\n\n---\n\n` +
        `⚠️ **Focus on the changed scenes**: the scenes above contain **newly added or modified content** since the last update. Please pay special attention to the new information in these scenes.\n`;
    } else {
      changedScenesContent = `\n\n⚠️ **No changed scenes**: all scenes were already analyzed in the last Persona update, so you may read all scenes directly for a global review.\n`;
    }
