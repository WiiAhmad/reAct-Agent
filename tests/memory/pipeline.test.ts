...
test("pipeline prunes expired store rows after a successful maintenance run", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "grammy-pipeline-"));
  ...
  try {
    ...
    await store.upsertL1({
      ...
      timestampStr: "2000-01-01T00:00:00.000Z",
      sourceConversationIds: [999],
      metadata: { source: "test" },
      createdTime: "2000-01-01T00:00:00.000Z",
      updatedTime: "2000-01-01T00:00:00.000Z",
    });
    await store.upsertL0({
      recordId: "legacy:l0:1",
      sessionKey: "telegram:c1:u1",
      sessionId: "c1",
      chatId: "c1",
      userId: "u1",
    ...
    const result = await new PipelineCoordinator(backend, fakeLlm, store, undefined, 1).runMaintenanceForUser("u1");
    const remainingL0 = await store.queryL0ForL1("telegram:c1:u1", 0, 10);
    const remainingL1 = await store.queryL1Records({ userId: "u1", type: "L1", limit: 10 });

    expect(result.personaUpdated).toBe(true);
    expect(remainingL0.some((row) => row.recordId === "legacy:l0:old")).toBe(false);
    expect(remainingL1.some((row) => row.recordId === "legacy:l1:old")).toBe(false);
    expect(remainingL1.some((row) => row.content === "User prefers Bun runtime")).toBe(true);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
